...
if [ -z "$TAG" ]; then
	...
	# depends on the operating system. Existing Ubuntu XX.04 users need
	# to be able to upgrade to the latest version supporting Ubuntu XX.04,
	# in part because an upgrade is required before jumping to Ubuntu XX.04.
	# New users on Ubuntu 20.04 need to get the latest version number too.
	#
